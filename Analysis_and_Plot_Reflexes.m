function Table = Analysis_and_Plot_Reflexes(trial,Plot)
% This function reads in raw EMG and kinematic data, processes it and will
% produce figures if Plot term is set to 'Y' or 'Yes' (otherwise graphs will
...
%            1st figure plots EMG, Motor data, and Ankle Angle through
%              time
%            2nd figure provides Fast Fourier Transform of raw &
%              filtered EMG data
%            3rd figure provides raw EMG with the flexion phase in blue and
%              extension in red, superimposed with joint angle in green in 
%              top plot. Bottom plot shows Fast Fourier Transform for each 
...
% Calculating Joint Angle (theta)
AB = [(K(1,1)-K(1,3)),(K(1,2)-K(1,4))];
CD = [(K(:,7)-K(:,5)),(K(:,8)-K(:,6))];
u1 = AB/sqrt(dot(AB,AB));
for i = 1:length(K)
    u2(i,:) = CD(i,:)/sqrt(dot(CD(i,:),CD(i,:)));
    theta(i,1) = acosd(dot(u1,u2(i,:)));
end
% Filtering Joint Angle
...
I(:,2) = 2*I(:,2);
I(:,3) = 0.5*I(:,3);
% Creating time vectors for each data point
...
%% FILTERING DATA
% Opted not to filter EMG signal because there appears to be an artifact
% within the range of biologically relevant frequencies of neurons so
% instead we are analyzing the raw EMG and will compare ratio of activation
% during ankle extension and flexion.
...
EMG_r = abs(I(:,1));    % FOR FILTERING EMG_r = abs(EMG_filt(:,1)); % Absolute Value of Filtered EMG
...
Idx_Ext = round(t_k(1,idx_ext)*10000); % row index of EMG signal when ankle extension begins to occur
Idx_End = round(t_k(1,idx_end)*10000); % row index of EMG signal when ankle extension stops (ends)
...
EMG_thresh = (AvgNoise + 3*SDnoise); % 5SD captures 99.9999% of data, so expected value of false positive in 0.1s interval is below 1
